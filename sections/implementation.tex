...
\cleardoublepage
\paragraph{Installation.} The \acrshort{cli} tool is installed by downloading a \lstinline|bash| script and piping it to the shell (listing \ref{lst:curlpipesh}). This pattern is informally known as \emph{``curl pipe sh''} and is commonly used because of its simplicity. However, its appearance may deter security-conscious users, a limitation which is discussed in section~\ref{sec:curlpipesh}. A content delivery network serves the installer script exclusively over \acrshort{http}S.
...
Then the value of the version tuple in each app's configuration file is overwritten with the generated version string. This ensures that the version of an application changes between different releases if and only if any files of that application were changed. Note that minor non-functional changes such as comments or formatting also result in a changed version string.
