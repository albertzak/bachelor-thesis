...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[>=latex']
    ...
    \path[densely dotted] (container.north west) edge (entrypoint.south west);
    \path[densely dotted] (container.north east) edge (command.south east);
  \end{tikzpicture}
...
\paragraph{Packaging and storing the release.} With a final, merged \acrshort{relup} file in place, the release is ready to be packaged as a tarball (\lstinline|*.tar.gz|), which contains everything needed to start a \emph{target system} from scratch, or to hot-upgrade a system running any of the versions of any branches that were present in the store when the build process was started. The resulting tarball artifact is sent to the store.
