...
\cleardoublepage
\paragraph{Installation.} The \acrshort{cli} tool is installed by downloading a \lstinline|bash| script and piping it to the shell (listing \ref{lst:curlpipesh}). This pattern is informally known as \emph{``curl pipe sh''} and is commonly used because of its simplicity. However, its appearance may deter security-conscious users, a limitation which is discussed in section~\ref{sec:curlpipesh}. A content delivery network ensures the installer script is served exclusively over \acrshort{http}S.
